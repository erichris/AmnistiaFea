<?php
  function getPosts($PostIds, $PostMessage){
    ...
    try {
      ...
      $response = $fb->get('me?fields=id,name,posts.limit(10){message,id}', $_SESSION['fb_access_token']);
    ...
    $filter = ["dVd", "obvio", "agotar", "desconocido"];
    $PostIds = [];
    $PostMessage = [];
  ...
  function createList($PostIds, $PostMessage){
    ...
    for($i = 0; $i < count($PostIds); $i++) {
      ...
      print("<div class='PostElement'><a class='PostList' href=\"" . $link . "\" target=\"popup\" onclick=\"window.open('$link','popup','width=600,height=600'); return false;\">" . $message . "</br></a></div>");
    }
